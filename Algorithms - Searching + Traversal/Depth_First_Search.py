...
class BinarySearchTree:
    ...
    #   PreOder = [9, 4, 1, 6, 20, 15, 170]
    # PostOrder = [1, 6, 4, 15, 170, 20, 9]

    def inOrderDFS(self, currentNode, DFSList):
        if currentNode.leftNode:  # Does currentNode have leftChild?
            self.inOrderDFS(currentNode.leftNode, DFSList)  # Go all the way down to currentNode.left | Keep going left to no more children

        DFSList.append(currentNode.data)  # Once at the end, add data to DFSList | Left side nodes get into list first

        if currentNode.rightNode:  # Does currentNode have rightChild?
            self.inOrderDFS(currentNode.rightNode, DFSList)  # Go all the way to the right to no more children

        return DFSList

    def preOrderDFS(self, currentNode, DFSList):
        DFSList.append(currentNode.data)  # Add data from the very beginning, before leftNode | parentNode before childNode

        if currentNode.leftNode:  # Does currentNode have leftChild?
            self.preOrderDFS(currentNode.leftNode, DFSList)  # Go all the way down to currentNode.left | keep going left to no more children

        if currentNode.rightNode:  # Does currentNode have rightChild?
            self.preOrderDFS(currentNode.rightNode, DFSList)  # Go all the way to the right to no more children

        return DFSList

    def postOrderDFS(self, currentNode, DFSList):
        if currentNode.leftNode:  # Does currentNode have leftChild?
            self.postOrderDFS(currentNode.leftNode, DFSList)  # Go all the way down to currentNode.left | keep going left to no more children

        if currentNode.rightNode:  # Does currentNode have rightChild?
            self.postOrderDFS(currentNode.rightNode, DFSList)  # Go all the way to the right to no more children

        DFSList.append(currentNode.data)  # Add data at the very end
...
bst.insert(1)
print()
print(f"in_OrderDFS => {bst.inOrderDFS(bst.root, [])}")
print(f"preOrderDFS => {bst.preOrderDFS(bst.root, [])}")
print(f"pstOrderDFS => {bst.postOrderDFS(bst.root, [])}")
